...
echo $i
echo "------"
snakemake --latency-wait 60 --config gene_list=${i} -j 100 --immediate-submit --cluster-config ../cluster.json --cluster "export PATH=/n/home04/xiaoleliu/ChiLin/alvin/xiaoleliu_lab/marge2/phaseI_init/miniconda3/bin:$PATH;source activate lisa; ../sbatch_script.py {dependencies}"
#exit 0
done
