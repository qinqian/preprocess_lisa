...
f = os.popen('wc -l ../MARGEData/histonerp/test/mm10.tss')
a = f.read()
f.close()
l = int(a.split()[0])
    ...
    if fin:
        ...
        if not os.path.exists(os.path.basename(fin[0])+'.txt'):
            ...
            if status == 0:
                ...
                _bw.getrp(fin[0], '../MARGEData/histonerp/test/mm10.tss', os.path.basename(fin[0])+'.txt', 1e4, 0, 0)
...
with h5py.File('margeRP_%s_mm.h5' % factor, 'a') as store:
    for f in files[1:]:
       print(n)
       n += 1
       tmp = []
