...
factors = {}
stat = {}
with open('margeFactor_mm.csv') as inf:
    inf.readline()
    for line in inf:
...
f = os.popen('wc -l mm10_window1kb.bed')
a = f.read()
f.close()
l = int(a.split()[0])
...
for key in factors:
    ...
    if fin:
        ...
        if not os.path.exists('%s.tab' % prefix):
            ...
            if status == 0:
                os.system('bigWigAverageOverBed %s mm10_window1kb.bed stdout > %s.tab  ' % (fin[0], prefix))
...
f = h5py.File('mm10_window1kb_%s.h5' % factor, 'a')
...
print(shape2)
print(len(files))
with h5py.File('mm10_window1kb_%s.h5' % factor, 'a') as store:
    for f in files:
       print(n)
       tmp = []
       with open(f) as inf:
